...
def compute_average_fitness_for_week(
  dataset,
  input_filename='results.json',
  output_path='output',
  output_filename='average_fitness_for_week.json',
):
  print('Computing average fitness for week across all departments...')
...
def plot_average_fitness_for_week(
  dataset,
  input_filenames=['average_fitness_for_week.json'],
  output_path='output',
  output_filename='average_fitness_for_week.png',
):
  year_weeks = dataset[YEAR_WEEK_DEPARTMENT_KEY].unique().tolist()
  year_weeks = [year_week.split('-')[0] + '-' + year_week.split('-')[1] for year_week in year_weeks]
  year_weeks = list(set(year_weeks))
  ...
  plt.title('Average Fitness for Week across all Departments')
...
# Computes the average fitness for each department
def compute_average_fitness_for_department(
  dataset,
  input_filename='results.json',
  output_path='output',
  output_filename='average_fitness_for_department.json',
):
  print('Computing average fitness for department...')
...
def plot_average_fitness_for_department(
  dataset,
  input_filenames=['average_fitness_for_department.json'],
  output_path='output',
  output_filename='average_fitness_for_department.png',
):
  departments = dataset[YEAR_WEEK_DEPARTMENT_KEY].unique().tolist()
  departments = [department.split('-')[2] for department in departments]
  departments = list(set(departments))
  ...
  plt.ylabel('Fitness')
  plt.title('Average Fitness for Department')
  plt.legend(title='Input Files', bbox_to_anchor=(1.2, 1), loc='lower right', borderaxespad=0.)
  plt.xticks(rotation=90)
